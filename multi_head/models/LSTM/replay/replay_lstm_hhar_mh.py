"""
LSTM with Experience Replay -- HHAR  (TIL Setting 2).

Same replay logic as the single-head HHAR runner: a fixed-size buffer stores
mem_size samples across past devices (quota mem_size // num_tasks each), and the
current device's data is concatenated with the buffer each task. Multi-head only
adds the per-device head routing (oracle task ID at eval).
Tasks = device models. Input: (B, 128, 3); 6 activity classes per head.
"""
import os, sys
import torch
...
def run_lstm_replay_hhar_mh(args, verbose=True, trial=None):
    """LSTM + Replay (MH) on HHAR. Returns val_accs, test_accs, metrics, model, []."""
    ...
    device    = args.device
    model     = MultiHeadLSTM(input_size, hidden_size, device, batch_size, num_classes=6)
    ...
    if verbose:
        print()
        print("  HHAR results  --  LSTM-Replay MH")
