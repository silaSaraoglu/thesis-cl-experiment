"""
Joint Multi-Head LSTM -- HHAR  (TIL Setting 2, upper bound).

All device heads added upfront; shared body + all heads trained simultaneously.
Tasks = device models; per-device 6-way head. Input: (B, 128, 3).
"""
...
def run_joint_lstm_hhar_mh(args, verbose = True):
    """Joint Multi-Head LSTM on HHAR. Returns val_accs, test_accs, metrics, model, []."""
    device      = args.device
    ...
    if verbose:
        ...
        print()
        print("  HHAR results  --  LSTM MH (Joint)  [oracle upper bound]")
