"""
LSTM with Experience Replay -- WISDM  (DIL Setting 1).
...
def run_lstm_replay_wisdm_sh(args, verbose=True, trial=None):
    """LSTM + Replay on WISDM. Returns val_accs, test_accs, metrics, model, []."""
    ...
    wisdm_train = WISDM_CL(data_dir, train=True, download=False, perc_val=0.25, batch_size=batch_size)
    ...
    for task_id in range(_NUM_TASKS):
        ...
        for eval_id in range(task_id + 1):
            ...
            loader_te = DataLoader(wisdm_test, batch_size=batch_size,
                                   shuffle=False, drop_last=False)
    ...
    if verbose:
        print()
        print("  WISDM results  --  LSTM-Replay")
