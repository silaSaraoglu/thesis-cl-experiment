"""
ESN with Experience Replay -- WISDM.

CL strategy: Experience Replay — a fixed-size ring buffer stores mem_size past samples
  and augments each mini-batch during new-task training.
...
def run_esn_replay_wisdm(args, verbose = True, trial=None):
    ...
    mem_size    = max(1, sum(int(len(d) * 0.1) for d in train_datasets))
    args_cl     = Namespace(**{**vars(args), "strategy": "replay", "mem_size": mem_size})
    ...
    if verbose:
        print(f"  WISDM results  --  ESN-Replay")
