...
CL strategy: Elastic Weight Consolidation — quadratic penalty on readout weights
  important to past tasks (λ·F·(θ-θ*)²).  ewc_mode='separate' keeps one Fisher per task.
...
def run_esn_ewc_wisdm(args, verbose = True, trial=None):
