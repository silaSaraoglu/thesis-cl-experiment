"""
ESN with LwF -- Sequential MNIST.

CL strategy: Learning without Forgetting (Li & Hoiem, 2017).
  Before training on a new task, soft predictions from the CURRENT model are
  recorded as distillation targets ("teacher"). During training the loss is:

      L = CrossEntropy(predictions, hard_labels)
        + alpha · KL(predictions / T, teacher_outputs / T)

  where T (temperature) softens both distributions for smoother gradients.
  This preserves old task behaviour without storing any past data.

  lwf_alpha       : weight of the distillation term (higher = more retention)
  lwf_temperature : softening factor for the probability distributions

Original repo: models/ESN/repo/experiments/splitmnist_esn.py  (strategy='lwf')
Avalanche LwF: avalanche.training.LwF
...
def run_esn_lwf_mnist(args, verbose = True, trial=None):
    ...
    # ── CL strategy: LwF ─────────────────────────────────────────────────────
    # Avalanche LwF snapshots the model before each task and adds distillation loss.
    args_cl     = Namespace(**{**vars(args), "strategy": "lwf"})
    ...
    for task_id, exp in enumerate(scenario.train_stream):
        ...
        # Avalanche LwF: snapshots current model, adds distillation loss during training
